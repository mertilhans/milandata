...
// Fonksiyon prototipleri (sizin başlık dosyanızdan)
//
// char *expand_with_quotes(char *line, t_env *env_list, int exit_status);
...
int heredoc_handle(t_parser *cmd_list,t_exec_data *data, t_env *env_list)
{
    ...
    while (current_cmd)
    {
        current_cmd->heredoc_fd = process_heredocs(current_cmd, env_list, data->last_exit_status);
...
int data_init(t_exec_data *data, t_parser *cmd_list,char **env ,int *pid_len)
{
    ...
    *pid_len = count_commands(cmd_list);
    ...
    data->env = env;
    data->in_fd = STDIN_FILENO;
    data->pipefd[0] = -1;  // Initialize pipe fds
    data->pipefd[1] = -1;
...
int execute_cmds(t_parser *cmd_list, char **env, t_env **env_list)
{
    ...
    {
        data.last_exit_status = n_next_or_built(cmd_list, &data, env_list);
        return (data.last_exit_status);
    }
