...
/*   Updated: 2025/08/01 02:24:00 by merilhan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "shell.h"

// tokenizer'ı başlat abi - input stringini alıp parse etmeye hazırla
...
// Helper function to access export list
...
int main(int ac , char **av, char **env)
{
	...
	char shlvl_str[16];
	sprintf(shlvl_str, "%d", shlvl_value);
	...
	while(1)
	{
		...
		if(tokens)
		{
			...
			// Update env array from current env_list
			char **updated_env = env_list_to_array(env_list);
			if (updated_env)
			{
				execute_cmds(cmd_list, updated_env, env_list);
				// Free updated env array
				for (int i = 0; updated_env[i]; i++)
					free(updated_env[i]);
				free(updated_env);
			}
			else
		...
		// readline() system malloc kullanır, manual free gerekli
		free(line);
	}
    
    // Normal program exit - OS will clean up memory
    return (0);
}