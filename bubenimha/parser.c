...
void ft_redir_in(t_token *tokens, t_parser *cmd, t_env *env_list)
{
    tokens = tokens->next;
    if (tokens && tokens->type == TOKEN_WORD)
    {
        char *expanded = expand_with_quotes(tokens->value, env_list);
        if (expanded)
...
void ft_redir_out(t_token *tokens, t_parser *cmd, t_env *env_list)
{
    tokens = tokens->next;
    if (tokens && tokens->type == TOKEN_WORD)
    {
...
void ft_redir_append(t_token *tokens, t_parser *cmd, t_env *env_list)
{
    tokens = tokens->next;
    if (tokens && tokens->type == TOKEN_WORD)
    {
...
void ft_redir_heredoc(t_token *tokens, t_parser *cmd, t_env *env_list)
{
    (void)env_list;  
    tokens = tokens->next;
    if (tokens && tokens->type == TOKEN_WORD)
...
void ft_loop_3(t_token **tokens, t_parser *cmd, int *argc, t_env *env_list)
{
    int i;
    char **new_argv;
    ...
    if ((*tokens)->type == TOKEN_WORD)
    {
        ...
        char *expanded = expand_with_quotes(original_token, env_list);
            ...
            (ft_strchr(expanded, ' ') || ft_strchr(expanded, '\t')))
        {
            ...
            {
                int value = 0;
                while(split_args[value])
                {
                    gc_free(split_args[value]);
                    value++;
                }
            }
            gc_free(expanded);
...
void ft_loop_2(t_token **tokens, t_parser *cmd,int *argc, t_env *env_list)
{
    if ((*tokens)->type == TOKEN_REDIR_APPEND)
    {
        ft_redir_append(*tokens, cmd, env_list);
    ...
    {
        ft_redir_heredoc(*tokens, cmd, env_list);
        if ((*tokens)->next && (*tokens)->next->type == TOKEN_WORD)
            *tokens = (*tokens)->next;
        else
            printf( "bash: syntax error near unexpected token `newline'\n");
        ...
        ft_loop_3(tokens,cmd,argc, env_list);

}

void ft_loop(t_token **tokens, t_parser *cmd, int *argc, t_env *env_list)
{
    if (cmd->parse_error)
    ...
    {
        ft_redir_in(*tokens, cmd, env_list);
        if ((*tokens)->next && (*tokens)->next->type == TOKEN_WORD)
            *tokens = (*tokens)->next;
        else 
            printf( "bash: syntax error near unexpected token `newline'\n");
    ...
    {
        ft_redir_out(*tokens, cmd, env_list);
        if ((*tokens)->next && (*tokens)->next->type == TOKEN_WORD)
            *tokens = (*tokens)->next;
        else 
            printf( "bash: syntax error near unexpected token `newline'\n");
        ...
        ft_loop_2(tokens,cmd,argc, env_list);

    *tokens = (*tokens)->next;
}

t_token *ft_control_token(t_token *tokens, t_parser **cmd_list, t_parser **last_cmd, t_env *env_list)
{
    int argc;
    t_parser *cmd;
    ...
    while (tokens && tokens->type != TOKEN_PIPE && tokens->type != TOKEN_EOF)
        ft_loop(&tokens, cmd, &argc, env_list);
    cmd->argv[argc] = NULL;
    if (!(*cmd_list))
        *cmd_list = cmd;
    else
...
t_parser *parse_tokens(t_token *tokens, t_env *env_list)
{
    ...
    while (tokens && tokens->type != TOKEN_EOF )
    {
        ...
        if (!tokens || tokens->type == TOKEN_EOF)
            break;
        tokens = ft_control_token(tokens, &cmd_list, &last_cmd, env_list);
    }
    return cmd_list;
}