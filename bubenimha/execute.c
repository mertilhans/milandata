...
char *ft_readline_loop(t_reader *r)
{
    ...
    {
        // Check for heredoc interruption after each character
    ...
    // Check for interruption when read fails
    if (is_heredoc_interrupted())
    {
        gc_free(r->buf);
        return NULL;
...
char *expand_heredoc_line_impl(char *line, t_env *env_list)
{
    ...
    if (!line)
        return NULL;
    expanded = expand_with_quotes(line, env_list);
    if (expanded)
        return expanded;
    else
...
char *readline_loop(t_heredoc_buffer *buf, const char *delimiter)
{
    ...
    while (1)
    {
        ...
        // Check for Ctrl+C interruption first
        if (is_heredoc_interrupted())
        {
            set_heredoc_interrupted(0); // Reset flag
            setup_interactive_signals();
            return ft_strdup(""); // Return empty string, not NULL
        }
        
        // Then check for Ctrl+D (EOF)
        if (!buf->line)
        {
...
char *read_single_heredoc_block(char *delimiter)
{
    ...
    // Check if heredoc was interrupted by Ctrl+C
    if (is_heredoc_interrupted())
    {
        set_heredoc_interrupted(0); // Reset flag
...
int ft_h_built_expand(t_redirection *current_redir,t_heredoc_data *data,t_env *env_list)
{
    ...
    //// FD_CLOEXEC flag'i ekle (execve'de otomatik kapanır)
    //fcntl(pipefd[0], F_SETFD, FD_CLOEXEC);
    //fcntl(pipefd[1], F_SETFD, FD_CLOEXEC);
    write(pipefd[1], data->heredoc_content, data->heredoc_len);
    close(pipefd[1]); // Write end'ini kapat
    gc_free(data->heredoc_content);
...
{   
    ...
    {
        cd_result = built_cd(cmd);
        set_last_exit_status(cd_result);
        return (0);
    }
...
int ft_redir_ctrl(t_parser *cmd)
{
    ...
    while (redir)
    {
        ...
        {
            // Heredoc input'u sadece başka input redirection yoksa kullan
            // Pipeline'da pipe input öncelikli
    ...
    // Heredoc input'u en son işle (output redirection'ı override etmez)
    if (cmd->heredoc_fd != -1 && cmd->heredoc_fd != -2)
    {
        if (dup2(cmd->heredoc_fd, STDIN_FILENO) == -1)
        {
...
void ft_exec_start(t_parser *cmd,t_exec_data *data, t_env **env_list)
{
    ...
    // SIGPIPE'ı ignore et - Bu sayede broken pipe yazma hatalarını önler
    ...
    else
    {
        ...
        // execve öncesi SIGPIPE'ı default'a döndür 
        // Ama stderr'i /dev/null'a yönlendir broken pipe mesajlarını engellemek için
        ...
        // execve başarısız olursa stderr'i geri yükle
        if (stderr_fd != -1) {
...
void child_process_exec(t_parser *cmd, t_exec_data *data, t_env **env_list)
{
    ...
    // Eğer heredoc varsa pipe input'u kurma (heredoc öncelikli)
    if (cmd->heredoc_fd == -1 || cmd->heredoc_fd == -2)
        fd_not_stdin(data);
    else
    {
        // Heredoc varsa pipe input'u kapat
        if (data->in_fd != STDIN_FILENO)
            close(data->in_fd);
    }
    
    // Pipe kurulduysa ve sonraki komut varsa output'u ayarla
...
{
    // Sonraki komut yoksa pipe kurmaya gerek yok
    if (!cmd->next)
        return (0);
        
    // Her zaman pipe kur - heredoc sadece input'u etkiler, output'u değil
    ...
    // FD_CLOEXEC flag'i ekle - execve sırasında otomatik kapanacak
    fcntl(data->pipefd[0], F_SETFD, FD_CLOEXEC);
    fcntl(data->pipefd[1], F_SETFD, FD_CLOEXEC);
...
int fork_pipe_exec(t_parser *cmd, t_exec_data *data, t_env **env_list)
{
    ...
    if (pid == 0)
    {
        // Child process - SIGPIPE'ı ignore et
        signal(SIGPIPE, SIG_IGN);
...
{
    // Her komut için pipe fd'lerini reset et
    data->pipefd[0] = -1;
    data->pipefd[1] = -1;
    ...
    if (cmd->next)
    {
        // Pipe kurulduysa, parent write end'ini kapat, read end'ini sakla
        if (data->pipefd[0] != -1)
        {
            close(data->pipefd[1]); // Parent write end'ini kapat
            data->in_fd = data->pipefd[0]; // Read end'ini sonraki komut için sakla
        }
        else
        {
...
int execute_cmds(t_parser *cmd_list, char **env, t_env **env_list)
{
    ...
    {
        int exit_status = n_next_or_built(cmd_list, &data, env_list);
        return (exit_status);
    }
