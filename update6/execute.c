	...
	while (current_redir)
	{
		if (current_redir->type == REDIR_HEREDOC)
		{
			...
			if(ft_h_built_expand(current_redir,heredoc_content,last_heredoc_fd,heredoc_len, env_list, exit_status) == -1)
				return (-1);
			//printf("DEBUG: ft_h_built_expand succeeded for heredoc\n");
		}
...
int process_heredocs(t_parser *cmd, t_env *env_list, int exit_status)
{
	...
	if(h_loop(cmd,heredoc_content,&last_heredoc_fd,&heredoc_len, env_list, exit_status) == -1)
		return (-1);
	//printf("DEBUG H LOOP SUCCEEED RETURNİNG fd %d\n,",last_heredoc_fd);
...
{
	t_redirection *redir;
	int has_output_redir = 0;  // Output redirection var mı kontrolü
	...
	while (redir)
	{
		...
		{
			has_output_redir = 1;  // Output redirection var
		...
		{
			// Heredoc işlemi yapılmış, fd hazır
			// Ancak eğer output redirection varsa, heredoc fd'yi hemen kullanmıyoruz
			if (!has_output_redir && cmd->heredoc_fd != -1 && cmd->heredoc_fd != -2)
			{
				if (dup2(cmd->heredoc_fd, STDIN_FILENO) == -1)
				{
					perror("dup2 cmd->heredoc_fd");
					close(cmd->heredoc_fd);  // Hata durumunda kapat
					cmd->heredoc_fd = -1;
					return (-1);
				}
	...
	// Eğer output redirection varsa ve heredoc fd açıksa, şimdi uygula
	if (has_output_redir && cmd->heredoc_fd != -1 && cmd->heredoc_fd != -2)
	{
		if (dup2(cmd->heredoc_fd, STDIN_FILENO) == -1)
		{
...
{
	// Fork yapmadan önce heredoc fd'yi parent'ta kapat
	// Child process kendi kopyasını kapatacak
	if (setup_and_fork(cmd, data, env_list) == -1)
		return 0;
	
	// Fork sonrası parent process'te heredoc fd'yi kapat
...
int heredoc_handle(t_parser *cmd_list,t_exec_data *data, t_env *env_list, int exit_status)
{
	...
	//printf("DEBUG: process_heredocs returned fd: %d\n", current_cmd->heredoc_fd);
	if (current_cmd->heredoc_fd == -1)
	{
    	//printf("DEBUG: heredoc failed, setting exit status to 130\n");
...
int execute_cmds(t_parser *cmd_list, char **env, t_env **env_list)
{
	...
	{
		 close(data.original_stdin);   // ← EKLE
    close(data.original_stdout);  // ← EKLE
    return (data.last_exit_status);
	}
