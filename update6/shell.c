...
/*   Updated: 2025/08/06 15:22:04 by merilhan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "shell.h"

static int *get_exit_status_ref(void)
{
...
{
	return (*get_exit_status_ref());
}

void set_last_exit_status(int status)
{
	*get_exit_status_ref() = status;
}
...
int	process_command_line(char *line, t_env **env_list, char **env)
{
	...
	cmd_list = parse_tokens(tokens, *env_list, get_last_exit_status());
	if (!cmd_list)
	{
		//printf("DEBUG: cmd_list is NULL, line was: '%s'\n", line);
	...
	if (updated_env)
	{
		...
		//printf("DEBUG: execute_cmds returned: %d\n", exit_status);
	...
	//printf("DEBUG: Set global exit status to: %d\n", exit_status);
	return (exit_status);
}
