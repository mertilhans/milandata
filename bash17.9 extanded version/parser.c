...
void add_redirection(t_parser *cmd, t_redir_type type, char *filename)
{
    ...
    new_redir->type = type;
    new_redir->no_expand = 0; // Default: 0 başlatmazsam patlıyoruz bazı yerlerde
    new_redir->next = NULL;
...
// Heredoc için expansion flag ile redirection ekle önmli
...
void ft_loop_3(t_token **tokens, t_parser *cmd, int *argc, t_env *env_list, int exit_status)
{
    ...
    if ((*tokens)->type == TOKEN_WORD)
    {
        ...
        int has_dollar_expansion = 0;
        if (expanded && expanded != (*tokens)->value)
        {
            // $ expansion olup olmadığını kontrol et
            char *original = (*tokens)->value;
            int j = 0;
            char quote_char = 0;
            ...
            while (original[j])
            {
                ...
                {
                    if (quote_char == 0)
                        quote_char = original[j];
                    else if (quote_char == original[j])
                        quote_char = 0;
                }
                else if (original[j] == '$' && quote_char != '\'')
                {
                    has_dollar_expansion = 1;
                    break;
                }
                j++;
            }
        ...
        // Only split if there was actual $ expansion AND it contains spaces
        if (has_dollar_expansion && expanded && 
