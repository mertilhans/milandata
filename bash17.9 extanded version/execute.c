...
// execution datası ve pipe işlemleri için gerekli bilgiler update ettim
...
// directory string kopyalama  PATH aramada kullanıyoruz update ettim
...
int is_exec_other(char *path)
{
    ...
    // Eğer directory ise executable değil // kafayı yedim
...
// PATH değişkeninde komut ara abi  en önemli fonksiyon // v2.0 update
...
{
	(void)env_list; // parçalayamadım böyle yaptım inş düzeltiriz
...
int execute_cmds(t_parser *cmd_list, char **env, t_env **env_list)
{
    ...
    data.last_exit_status = 0; // bunu da struct olarak tanımladım nt yani
