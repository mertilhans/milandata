...
run_test "Dollar in Quotes" "echo 'HOME variable: \$HOME'\nexit" "HOME variable: \$HOME" "Dollar sign in single quotes"
...
run_test "PATH Göster" "echo \$PATH\nexit" "/usr" "PATH environment variable expansion"
run_test "HOME Göster" "echo \$HOME\nexit" "/home" "HOME directory variable expansion"
run_test "Yeni Değişken Export" "export TEST=merhaba\necho \$TEST\nexit" "merhaba" "Custom environment variable creation"
run_test "Export Birden Fazla" "export VAR1=test VAR2=deneme\necho \$VAR1 \$VAR2\nexit" "test deneme" "Multiple export handling"
run_test "Export Overwrite" "export PATH=/test\necho \$PATH\nexit" "/test" "Environment variable overwrite"
run_test "Export ile Boşluk" "export 'SPACE_VAR=hello world'\necho \$SPACE_VAR\nexit" "hello world" "Export with spaces"
run_test "Unset Sonrası" "export TEMP=test\nunset TEMP\necho \$TEMP\nexit" "" "Variable unset functionality"
run_test "Export Overwrite Multiple" "export VAR=first\nexport VAR=second\necho \$VAR\nexit" "second" "Variable overwrite test"
run_test "Export Boş Değer" "export EMPTY=\necho 'value:'\$EMPTY'end'\nexit" "value:end" "Empty variable export"
