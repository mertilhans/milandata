...
void child_process_exec(t_parser *cmd, t_exec_data *data, t_env **env_list)
{
    ...
    else if (cmd->next && cmd->next->heredoc_fd != -1 && cmd->next->heredoc_fd != -2)
    {
        // Sonraki komutun heredoc'u varsa bu komutun çıktısını /dev/null'a yönlendir
...
int setup_pipe_if_needed(t_parser *cmd, t_exec_data *data)
{
    ...
    // Sonraki komutun heredoc'u varsa pipe kurma
    // Çünkü heredoc pipe input'u override edecek
    if (cmd->next->heredoc_fd != -1 && cmd->next->heredoc_fd != -2)
        return (0);
...
int process_command(t_parser *cmd, t_exec_data *data, t_env **env_list)
{
    ...
    if (cmd->next)
    {
        ...
        {
            // Pipe kurulmadı (sonraki komutun heredoc'u var)
