...
/*   Updated: 2025/08/10 15:39:25 by merilhan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char *push_char_res(char *res, char c, int *res_len, int *res_cap)
{
    ...
    {
        *res_cap *= 2;
        char *new_res = gb_malloc(*res_cap);
        if (!new_res)
            return res;
        if(res)
...
char    *expand_heredoc_line(char *str,t_env *env_list, int exit_status)
{
    ...
    while(str[i])
    {
        if(str[i] == '\'' || str[i] == '\"')
        {
            ...
            result = push_char_res(result,str[i],&res_len,&res_cap);
            i++;
            if(quote_char == 0) // başlangıcı tutucaz
                quote_char = current_quote;
            else if (quote_char == current_quote)
                quote_char = 0;
